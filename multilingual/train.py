...
import loader
from loader import SpellingDataset, char_vocab
...
if not params.dummy_run and params.wandb:
    wandb.init(project="multilingual", name=run_name)
...
def evaluate(model, dataset, criterion):
    ...
    print(Counter(predicts), Counter(gnd_truths))
...
print(len(dataset)," datasets of Sizes:", len(dataset[char_vocab[0]][0]), len(dataset[char_vocab[0]][1]))
...
class SpellingModel(nn.Module):
    def __init__(self):
        ...
        if params.control:
            ...
            self.model_config = AutoConfig.from_pretrained(params.model_card)
            ...
            assert trained_embeddings[0] in ['embeddings.word_embeddings.weight', 'wte.weight', 'shared.weight']
...
assert len(test_dicts) == len(char_vocab)
...
test_dicts_aggr = {k1:
                    {k2: np.mean([single_dict[k1][k2]
                        ...
                        for k2 in test_dicts[char_vocab[0]][k1].keys()}
                    if type(test_dicts[char_vocab[0]][k1]) == dict
                    else np.mean([single_dict[k1] for single_dict in test_dicts.values()])
                for k1 in test_dicts[char_vocab[0]].keys()}
